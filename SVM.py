...
def main(**kwargs):
	...
	#Sheanan Verma (SVM Code with 80-20 Split)
	li=[]
	tar=[]
	#li.append(obj)
	print (len(datasets))
	for index in range(len(datasets['train'])):
		...
		#li.append(obj)
		a = np.array((datasets['train'][index]['feature']))
		#print (a)
		li.append(list(a.mean(axis=0)))
		#print(len(list((a))))
		tar.append(datasets['train'][index]['target'])
		#print (a.mean(axis=1))
		
		
	X_train, X_test, y_train, y_test = train_test_split(li,tar, test_size=0.2)
	clf = svm.SVC(gamma='scale')
	#print(type(y_train))
	#print(len(X_train))
	#print(len(y_train))
	clf.fit(X_train,y_train)
	print ("Final Score == ",clf.score(X_test, y_test, sample_weight=None)) 
